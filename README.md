...
- **The office sale (DC-21)**: sell milk from an admin action without a truck. Prices it, applies an administrative fee on every litre (tunable through SettingsHub, set per 1000 L), and refuses outright when the fee meets or beats the price, before any milk moves. Otherwise it removes the milk by the standard removal path and credits the money.
...
- **SettingsHub** — five admin settings (enable, default collection interval, spoilage, contracts, milk sale fee per 1000 L).
